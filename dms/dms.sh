...
source_endpoint_arn=$(aws dms create-endpoint --endpoint-identifier from-postgres-$1-db-$2 --endpoint-type source --engine-name postgres \
...
target_endpoint_arn=$(aws dms create-endpoint --endpoint-identifier to-aurora-$1-db-$new_db_name --endpoint-type target --engine-name aurora-postgresql \
...
aws dms create-replication-task --replication-task-identifier migrate-$1-postgres-db-to-aurora-$2-db --source-endpoint-arn $source_endpoint_arn --target-endpoint-arn $target_endpoint_arn \
...
replication_task_arn=$(aws dms describe-replication-tasks --filters "Name= replication-task-id,Values=migrate-$1-postgres-db-to-aurora-$2-db" --query "ReplicationTasks[0].ReplicationTaskArn" --output text)
